...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "update_amount") {
    ...
    if (empty($order_id) || $amount < 0) {
        ...
        if (mysqli_stmt_execute($upd)) $success = "Amount updated for order <strong>#$order_id</strong>.";
        else $error = "Failed to update amount.";
...
<title>TronSport Medicamon | Orders</title>
...
<style>
  ...
  .modal-overlay{display:none;position:fixed;inset:0;z-index:100;
    background:rgba(0,0,0,0.45);backdrop-filter:blur(4px);align-items:center;justify-content:center;}
...
<header class="bg-white/80 backdrop-blur-lg shadow-sm sticky top-0 z-50 flex justify-between items-center px-6 py-3 w-full">
  <div class="flex items-center gap-8">
    ...
    <nav class="hidden md:flex items-center gap-6">
      ...
      <a class="text-blue-700 font-bold border-b-2 border-blue-600 px-1 py-1" href="admin_orders.php">Orders</a>
...
<div class="flex min-h-screen">
  ...
  <aside class="bg-slate-50 h-screen w-64 border-r border-slate-200 flex flex-col gap-2 p-4 fixed left-0 top-[60px] hidden lg:flex">
    ...
    <nav class="flex-1 flex flex-col gap-1">
      ...
      <a href="admin_orders.php" class="bg-blue-50 text-blue-700 rounded-lg font-bold flex items-center gap-3 px-3 py-2.5 hover:translate-x-1 transition-transform">
    ...
    <div class="fade-in bg-surface-container-lowest rounded-2xl border border-outline-variant/15 shadow-sm overflow-hidden">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-6 border-b border-outline-variant/15">
        <div>
          ...
          <p class="text-xs text-on-surface-variant mt-0.5"><?php echo $totalOrders; ?> total</p>
        </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-outline-variant/10">
            ...
            ?>
            <tr class="hover:bg-surface-container-low/60 transition-colors order-row <?php echo $fc; ?>"
                data-status="<?php echo $isDel?'delivered':'pending'; ?>"
                data-assigned="<?php echo $isAsgn?'assigned':'unassigned'; ?>">
              <td class="px-5 py-4">
                <span class="font-mono font-bold text-primary">#<?php echo htmlspecialchars($ord["Tracking"]); ?></span>
                <?php if($isUrg): ?><span class="ml-1.5 text-[9px] font-black bg-error text-white px-1.5 py-0.5 rounded uppercase">URGENT</span><?php endif; ?>
              </td>
              ...
              <td class="px-5 py-4">
                <form method="POST" class="flex items-center gap-1.5">
                  ...
                  <input type="number" name="amount" value="<?php echo intval($ord["otalAmount"]); ?>" min="0"
                    class="w-24 px-2 py-1.5 bg-surface-container-high border-none rounded-lg text-sm font-semibold focus:ring-2 focus:ring-primary/20"/>
              ...
              <td class="px-5 py-4">
                <?php if($isAsgn): ?>
                  <div class="flex items-center gap-2">
                    ...
                    <span class="font-semibold text-on-surface text-xs"><?php echo htmlspecialchars($ord["dp_first"]." ".$ord["dp_last"]); ?></span>
                  </div>
                <?php else: ?>
              ...
              <td class="px-5 py-4 text-right">
                <button onclick="openAssignModal('<?php echo htmlspecialchars($ord["Tracking"]); ?>','<?php echo htmlspecialchars($ord["assigned_pharmacy"]??""); ?>','<?php echo htmlspecialchars($ord["deliveryperson_id"]??""); ?>')"
                  ...
                  <?php echo $isAsgn?"Reassign":"Assign"; ?>
                </button>
              </td>
            </tr>
            <?php endforeach; ?>
    ...
    <form method="POST" class="space-y-5">
      ...
      <div>
        ...
        <div class="space-y-2 max-h-64 overflow-y-auto pr-1" id="dp-radio-list">
          <?php foreach($deliveryPersons as $dp):
            $ini=strtoupper(substr($dp["FirstName"],0,1).substr($dp["LastName"],0,1));
          ?>
          <label class="flex items-center gap-3 p-3 rounded-xl border border-outline-variant/20 hover:bg-surface-container-low cursor-pointer transition-colors has-[:checked]:border-primary has-[:checked]:bg-primary/5">
            <input type="radio" name="dp_phone" value="<?php echo htmlspecialchars($dp["PhoneNumber"]); ?>" class="accent-primary w-4 h-4"/>
            <div class="w-9 h-9 rounded-full bg-gradient-to-br from-primary to-primary-container flex items-center justify-center text-white font-bold text-sm flex-shrink-0"><?php echo $ini; ?></div>
            <div>
      ...
      <div class="flex gap-3 pt-2">
        <button type="button" onclick="closeModal('assignModal')"
          class="flex-1 py-3 rounded-xl border border-outline-variant text-on-surface-variant font-bold text-sm hover:bg-surface-container transition-colors">Cancel</button>
        <?php if(!empty($deliveryPersons)): ?>
        <button type="submit"
