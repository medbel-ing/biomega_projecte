...
// ── Guard: delivery manager only ──────────────────────────────────────────────
if (!isset($_SESSION["table"]) || $_SESSION["table"] !== "deliverymanager") {
    header("Location: login.php");
    exit();
}

$firstname   = htmlspecialchars($_SESSION["firstname"] ?? "Manager");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["force_gps_phone"])) {
    ...
    $mgrName     = mysqli_real_escape_string($conn, $managerName);
    mysqli_query($conn,
        "INSERT INTO delivery_location (PhoneNumber, Latitude, Longitude, Status, GpsForced, ForcedAt, ForcedByAdmin)
         VALUES ('$targetPhone', 0, 0, 0, 1, NOW(), '$mgrName')
         ON DUPLICATE KEY UPDATE
           GpsForced = 1,
           ForcedAt = NOW(),
           ForcedByAdmin = '$mgrName'"
    );
    header("Location: deliverymanager_tracking.php?forced=" . urlencode($targetPhone));
    exit();
}
...
$deliveryPersons = [];
while ($row = mysqli_fetch_assoc($deliveryPersonsRes)) $deliveryPersons[] = $row;
...
<div class="flex min-h-screen">
  ...
  <aside class="bg-slate-50 h-screen w-64 border-r border-slate-200 flex flex-col gap-2 p-4 fixed left-0 top-[60px] hidden lg:flex">
    ...
    <nav class="flex-1 flex flex-col gap-1">
      ...
      <a href="delivery_manager_dashboard.php#orders-section"
         class="text-slate-600 hover:bg-slate-100 flex items-center gap-3 px-3 py-2.5 rounded-lg hover:translate-x-1 transition-transform">
        <span class="material-symbols-outlined">package_2</span><span class="text-sm">Orders</span>
      </a>
      <a href="delivery_manager_dashboard.php#dp-section"
         class="text-slate-600 hover:bg-slate-100 flex items-center gap-3 px-3 py-2.5 rounded-lg hover:translate-x-1 transition-transform">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-[360px_1fr] gap-5 fade-in">
      ...
      <div class="bg-surface-container-lowest rounded-2xl border border-outline-variant/15 shadow-sm flex flex-col overflow-hidden">
        ...
        <div class="overflow-y-auto flex-1 divide-y divide-outline-variant/10">
          ...
          ?>
          <div class="dp-card p-4 border border-transparent rounded-xl mx-2 my-1"
               data-phone="<?php echo htmlspecialchars($phone); ?>"
               data-lat="<?php echo $dp['Latitude'] ?? ''; ?>"
               data-lng="<?php echo $dp['Longitude'] ?? ''; ?>"
            ...
            <div class="mt-2 space-y-1">
              ...
              <?php endforeach; ?>
              <?php if(count($orders)>2): ?>
              <p class="text-[10px] text-primary font-bold text-right">+<?php echo count($orders)-2; ?> more</p>
              <?php endif; ?>
            </div>
            <?php endif; ?>
            ...
            <!-- Force GPS Button -->
            <?php if (!$isOnline): ?>
            <form method="POST" action="deliverymanager_tracking.php"
                  onsubmit="return confirmForce('<?php echo htmlspecialchars($dp['FirstName'].' '.$dp['LastName']); ?>')"
...
<nav class="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 pb-6 pt-3 bg-white/90 backdrop-blur-xl border-t border-slate-200">
  <a href="delivery_manager_dashboard.php" class="flex flex-col items-center text-slate-400">
    <span class="material-symbols-outlined">grid_view</span>
    <span class="text-[10px] font-semibold uppercase">Dashboard</span>
  </a>
  <a href="delivery_manager_dashboard.php#orders-section" class="flex flex-col items-center text-slate-400">
    <span class="material-symbols-outlined">package_2</span>
    <span class="text-[10px] font-semibold uppercase">Orders</span>
  </a>
  <a href="deliverymanager_tracking.php" class="flex flex-col items-center bg-blue-100 text-blue-800 rounded-xl px-3 py-1.5">
...
function renderAll() {
  ...
  DELIVERY_PERSONS.forEach(dp => {
    ...
    const forceBtnHtml = !isOnline
      ? `<form method="POST" action="deliverymanager_tracking.php" onsubmit="return confirmForce('${dp.FirstName} ${dp.LastName}')">
